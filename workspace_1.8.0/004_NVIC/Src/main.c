...
int main(void)
{
	...
	// Enable output mode for port D pin PD12 and PD14
	GPIOD_MODER &= ~(BIT(25)|BIT(29));
	GPIOD_MODER |=  (BIT(24)|BIT(28));
...
int main()
{
	...
	EXTI_IMR |= BIT(0);					//	Enable Interrupt from EXTI0 line 0
	EXTI_RTSR |= BIT(0);				//	Rising trigger enabled.

	NVIC_ISER |= BIT(6);				//	Enable Interrupt in NVIC.
...
void EXTI0_IRQHandler(void)
{
    EXTI_IMR &= ~BIT(0);   // mask interrupt

    EXTI_PR |= BIT(0);     // clear pending
    ...
    EXTI_IMR |= BIT(0);    // re-enable interrupt
}
